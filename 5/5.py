...
for idx, i in enumerate(List):
    # Обучаем лес
    a = random_forest(train_data, train_labels, i)
    train_answers = tree_vote(a, train_data)
    test_answers = tree_vote(a, test_data)
    ...
    mesh_predictions = np.array(tree_vote(a, grid_points))
    mesh_predictions = mesh_predictions.reshape(xx.shape)
    
    # Создаем новую фигуру для каждой пары графиков
